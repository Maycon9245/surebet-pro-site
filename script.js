document.addEventListener("DOMContentLoaded", function () {
  ...
  function createCheckboxes(items, containerId, selectedStorageKey) {
    ...
    items.forEach(name => {
      ...
      input.addEventListener("change", () => {
        ...
        renderSurebets(applyFilters(lastFetched));
      });
  ...
  ["sportsSelectAll", "sportsClear", "bookiesSelectAll", "bookiesClear", "btnAtualizar"].forEach(id => {
    document.getElementById(id)?.addEventListener("click", () => {
      if (id === "btnAtualizar") fetchData();
        ...
        localStorage.setItem(isSports ? "selectedSports" : "selectedBookmakers", JSON.stringify(isSelect ? (isSports ? ["Futebol", "Basquete", "Tênis"] : ["Betfair", "Pinnacle", "Betano", "Bet365"]) : []));
        renderSurebets(applyFilters(lastFetched));
      }
    });
  });
  ...
  $tbody.addEventListener("click", (e) => {
    ...
    if (act === "open") {
      ...
      const stakeTotal = parseFloat($stakeTotal.value) || 0;
  ...
  document.querySelectorAll(".plan-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      let days;
      if (plan === "semanal") days = 7;
      else if (plan === "mensal") days = 30;
      else if (plan === "trimestral") days = 90;
      document.getElementById("pixValue").textContent = `R$ ${plan === "semanal" ? "29,90" : plan === "mensal" ? "79,90" : "199,90"}`;
      showTab("pix");
